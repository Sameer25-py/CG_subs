...
// Task 1: implement this function
  ...
  for(SceneLight* light : scene->lights){
    ...
    for(SceneLight* light : scene->lights){
      ...
      for(int i=0 ;i<n ;i++){
          Spectrum check = light->sample_L(hit_p,&wi,&distToLight,&pdf);

          Vector3D wi_w = w2o * wi;
          if(wi_w.z < 0)continue;

          if(!bvh->intersect(Ray(EPS_D*wi + hit_p,wi,distToLight))){
            hit= true;
            sum+= isect.bsdf->f(w_out,wi_w) * wi_w.z * check / pdf;
          }
...
// Task 2: implement this function
  ...
  Spectrum point_spec =isect.bsdf->sample_f(w_out,&wi,&pdf);

  float rr = max(0.0,1-20*point_spec.illum() + 0.07);

  if(coin_flip(rr))return Spectrum();

  Vector3D wi_w = o2w*wi;
  int depth = r.depth-1;
  Spectrum radiance = trace_ray(Ray(hit_p + (wi_w*EPS_D),wi_w,depth),isect.bsdf->is_delta());
  return radiance * fabs(wi.z) * point_spec/((1-rr)*pdf);
