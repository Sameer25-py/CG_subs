...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if(samples== 1){
    //single ray 
    Ray ray = camera->generate_ray((x+0.5)/(double)sampleBuffer.w,(y+0.5)/(double)sampleBuffer.h);
    ray.depth = max_ray_depth;
    out = trace_ray(ray,true);
    
  }
  else{
    for(int i=0;i<samples;i++){
       Vector2D rand_off = gridSampler->get_sample();
       Ray ray = camera->generate_ray((x+rand_off.x)/(double)sampleBuffer.w,(y+rand_off.y)/(double)sampleBuffer.h);
       ray.depth = max_ray_depth;
       out+= trace_ray(ray,true);
