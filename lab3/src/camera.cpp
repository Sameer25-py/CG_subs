...
Ray Camera::generate_ray(double x, double y) const { 
  ...
  Vector3D bot_left = Vector3D(-tan(radians(hFov)*0.5),-tan(radians(vFov)*0.5),-1);
  Vector3D top_right = Vector3D(tan(radians(hFov)*0.5),tan(radians(vFov)*0.5),-1);
  Vector3D to_wspace = Vector3D(bot_left.x + ((top_right.x-bot_left.x)*x),bot_left.y + ((top_right.y-bot_left.y)*y),-1);
  to_wspace = c2w * to_wspace;
  to_wspace.normalize();
  Ray ray =Ray(pos,Vector3D(to_wspace),fClip,0);
  ray.min_t = nClip;
  return ray;
}
