...
private:
  ...
  //Use this function to retrieve subpixel color. i and j are the indices of the subpixel with reference to the pixel.
    ...
    float no_samples = samples_per_side * samples_per_side;
    Color avg = Color(0,0,0,0);
    for(int i=0;i<samples_per_side;i++){
      for(int j=0;j<samples_per_side;j++){
          //dividing by pixel weight here otherwise whole brightness is effected
          avg+=get_subpixel_color(i,j)*(1/no_samples);
      }
    }            
    return avg; 
  }
